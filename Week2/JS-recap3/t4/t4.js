...
function userLocation() {
  navigator.geolocation.getCurrentPosition(function (position) {
    ...
    sortRestaurantsDistance(userLat, userLon);
  });
}
...
function sortRestaurantsDistance(userLat, userLon) {
  for (let rest of restaurants) {
    const lat2 = rest.location.coordinates[1];
    const lon2 = rest.location.coordinates[0];
    const distance = calculateDistance(userLat, userLon, lat2, lon2);
...
function displayRestaurants() {
  ...
  for (let rest of restaurants) {
    ...
    addressTd.textContent = rest.address;
